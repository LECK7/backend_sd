...
import { registrarLog } from '../middlewares/logger.js'; // 👈 importa el logger
...
router.post('/crear', requireAuth, async (req, res) => {
  ...
  try {
    const venta = await prisma.$transaction(async (tx) => {
      ...
      // crear items, decrementar stock y movimiento inventario
      for (const it of items) {
    ...
    await registrarLog(req, 'VENTA_CREADA', `Venta ${venta.id} creada por ${req.user.email}`);
